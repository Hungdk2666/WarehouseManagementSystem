...
<%@page import="model.PeriodSummaryRow"%>
...
<%
    ...
    List<PeriodSummaryRow> rows = (List<PeriodSummaryRow>) request.getAttribute("periodRows");
    ...
    long totalOpening = 0, totalImport = 0, totalExport = 0, totalAdjustment = 0, totalClosing = 0;
    if (rows != null) { for (PeriodSummaryRow r : rows) {
        totalOpening += r.getOpeningQuantity();
        totalImport += r.getImportQuantity();
        totalExport += r.getExportQuantity();
        totalAdjustment += r.getAdjustmentQuantity();
        totalClosing += r.getClosingQuantity();
    } }
%>
<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8">
    <title>Báo cáo tổng hợp Nhập - Xuất - Tồn - WMS</title>
...
<body>
        ...
        <div class="row">
            ...
            <div class="col-md-9 col-lg-10">
                <div class="page-header">
                    <div>
                        <h2 class="page-title">Báo cáo tổng hợp Nhập - Xuất - Tồn</h2>
                        <p class="page-subtitle">Đầu kỳ - phát sinh trong kỳ - cuối kỳ của từng vật tư, tách theo tình trạng hàng</p>
                    </div>
                    <% if (!fromDate.isEmpty() && !toDate.isEmpty()) { %>
                    <a href="<%= request.getContextPath() %>/warehouse/movement-report?type=period&action=export<%= filterParams %>"
                ...
                <ul class="nav nav-pills mb-3">
                    <li class="nav-item">
                        <a class="nav-link" href="<%= request.getContextPath() %>/warehouse/movement-report?type=daily">Chi tiết theo ngày</a>
                    </li>
                    <li class="nav-item">
                    ...
                    <li class="nav-item">
                        <a class="nav-link active" href="<%= request.getContextPath() %>/warehouse/movement-report?type=period">Tổng hợp Nhập - Xuất - Tồn</a>
                    </li>
                </ul>
                        ...
                        <form id="filterForm" action="movement-report" method="GET" class="row g-2 align-items-end">
                            ...
                            <div class="col-6 col-md-auto">
                                <div class="form-check mt-4">
                                    ...
                                    <label class="form-check-label small" for="includeZero">Hiện cả dòng không phát sinh</label>
                                </div>
                            </div>
                            <div class="col-12 col-md-auto ms-md-auto d-flex gap-2">
                                ...
                                <a href="movement-report?type=period" class="btn btn-outline-secondary btn-sm px-3">
                ...
                <div class="row g-2 mb-3">
                    <div class="col-xl col-6"><div class="card border-0 shadow-sm px-3 py-2"><div class="small text-muted">Đầu kỳ</div><div class="fs-5 fw-bold text-secondary"><%= nf.format(totalOpening) %></div></div></div>
                    <div class="col-xl col-6"><div class="card border-0 shadow-sm px-3 py-2"><div class="small text-muted">Nhập trong kỳ</div><div class="fs-5 fw-bold text-success"><%= nf.format(totalImport) %></div></div></div>
                    <div class="col-xl col-6"><div class="card border-0 shadow-sm px-3 py-2"><div class="small text-muted">Xuất trong kỳ</div><div class="fs-5 fw-bold text-danger"><%= nf.format(totalExport) %></div></div></div>
                    <div class="col-xl col-6"><div class="card border-0 shadow-sm px-3 py-2"><div class="small text-muted">Điều chỉnh</div><div class="fs-5 fw-bold text-secondary"><%= totalAdjustment > 0 ? "+" : "" %><%= nf.format(totalAdjustment) %></div></div></div>
                    <div class="col-xl col-6"><div class="card border-0 shadow-sm px-3 py-2"><div class="small text-muted">Cuối kỳ</div><div class="fs-5 fw-bold text-primary"><%= nf.format(totalClosing) %></div></div></div>
                </div>
                ...
                <div class="card bg-white">
                    <div class="card-header bg-white py-3 d-flex justify-content-between align-items-center border-bottom">
                        <span class="fw-bold text-slate-800">
                            ...
                            Kỳ từ <%= fromDate %> đến <%= toDate %> (<%= nf.format(totalRows) %> dòng)
                        </span>
                    </div>
                    <div class="card-body p-0">
                        <div class="table-responsive">
                            <table id="periodSummaryReportTable" class="table table-hover align-middle mb-0" style="min-width: 1220px;">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-end fw-bold"><%= nf.format(r.getClosingQuantity()) %></td>
                    ...
                    <div class="card-footer bg-transparent border-top d-flex flex-column flex-sm-row justify-content-between align-items-center px-4 py-3 gap-3">
                        <div class="d-flex align-items-center gap-2">
                            ...
                            <select id="periodSummaryEntriesPerPage" class="form-select form-select-sm border border-secondary-subtle bg-white shadow-none px-3 py-1" style="width: 80px; border-radius: 8px;">
                        ...
                        <div id="periodSummaryPaginationContainer" class="d-flex align-items-center justify-content-between justify-content-sm-end gap-3 flex-wrap w-100 w-sm-auto"></div>
                    </div>
                </div>
                <% } %>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            initPagination("periodSummaryReportTable", "periodSummaryPaginationContainer", "periodSummaryEntriesPerPage");
        });
